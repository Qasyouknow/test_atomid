...
PATH = r"D:\PythonProjects\pythonProject\PolyU\FYP\AtomIDNet\datasets"
...
class ImageLoader(Dataset):
    def __init__(self, data_root, file_root, mode:str="train", crop_size:int=256, is_syn:bool=False):
        ...
        for i in range(len(data)):
            img_name = data["image"].iloc[i].split('//')[-1].split('.')[0]
            img_path = Path(data_root, data["image"].iloc[i])
            csv_path = Path(data_root, data["image"].iloc[i].replace('.bmp', '.csv'))
...
# Load datasets
def get_tem_Data(data_root:str=PATH, file_root: str=PATH, crop_size:int=64, train_bs:int=4, num_workers: int=0, is_syn:bool=False):
