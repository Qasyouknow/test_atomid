...
class SyntheticTool():
    ...
    def add_bias(self, atoms:Atoms, indices=None, bias:int=1):
...
if __name__ == '__main__':
    root_path = '.'
    save_path = rf'{root_path}\datasets\syn'
    cif_path = rf'{root_path}\datasets\cif'
    ...
    for cif_file in tqdm(cif_path_list):
        name = cif_file.split('\\')[-1].replace(".cif", "")
        atoms = STool.read_atoms(cif_file)
        atoms = STool.expand_atoms(atoms, expand=np.random.randint(45, 75)/10)
        atoms = STool.add_vacancy(atoms, rate=0.05)
        atoms = STool.add_dopants(atoms, rate=0.02)
